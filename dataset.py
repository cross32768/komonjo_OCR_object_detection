...
class OCRDataset(Dataset):
    ...
    def anno2label(self, annotation_data, original_image_size):
        ...
        center_x = center_x * self.image_size / original_image_size['Width']
        center_y = center_y * self.image_size / original_image_size['Height']

        center_x_normalized = center_x / config.FE_STRIDE
        center_y_normalized = center_y / config.FE_STRIDE
        width_normalized = width / original_image_size['Width']
        height_normalized = height / original_image_size['Height']
...
class OCRDatasetWithAnchor(Dataset):
        ...
        for i in range(config.N_KINDS_OF_CHARACTERS):
            ...
            for annotation in self.annotation_list:
                ...
                if len(annotation_data) != 0:
                    ...
                    anchor_info = np.zeros((len(annotation_data_for_index), 2))
                    anchor_info[:, 0] = annotation_data_for_index[:, 3] / original_image_size['Width']
                    anchor_info[:, 1] = annotation_data_for_index[:, 4] / original_image_size['Height']
    ...
    def anno2label(self, annotation_data, original_image_size):
        ...
        char_indexes = annotation_data[:, 0]
        min_x = annotation_data[:, 1]
        min_y = annotation_data[:, 2]
        width = annotation_data[:, 3]
        height = annotation_data[:, 4]

        anchor_width = self.anchor_boxes[char_indexes, 0]
        anchor_height = self.anchor_boxes[char_indexes, 1]

        center_x = min_x + 0.5*width
        center_y = min_y + 0.5*height

        width = width / original_image_size['Width']
        height = height / original_image_size['Height']
        center_x = center_x * self.image_size / original_image_size['Width']
        center_y = center_y * self.image_size / original_image_size['Height']
        ...
        label[5*char_indexes + 0, center_y_grid, center_x_grid] = 1.0
        label[5*char_indexes + 1, center_y_grid, center_x_grid] = center_x_offset
        label[5*char_indexes + 2, center_y_grid, center_x_grid] = center_y_offset
        label[5*char_indexes + 3, center_y_grid, center_x_grid] = width_change_from_anchor
        label[5*char_indexes + 4, center_y_grid, center_x_grid] = height_change_from_anchor
